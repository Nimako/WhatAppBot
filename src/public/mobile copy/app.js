...
async function installPWA() {
    if (!deferredPrompt) {
        ...
        showToast('Install prompt not available. Check console for details.');
        return;
    }
...
console.log('- Is installed:', checkIfInstalled());

// Show install button if not installed (for manual testing)
if (!checkIfInstalled()) {
    setTimeout(() => {
        if (!deferredPrompt && installButton === null) {
            console.log('Install prompt not available. This might be because:');
            console.log('1. App is already installed');
            console.log('2. Browser doesn\'t support PWA installation');
            console.log('3. Missing icons in manifest (create icon-192.png and icon-512.png)');
            console.log('4. Not served over HTTPS (required for production)');
        }
    }, 2000);
}
